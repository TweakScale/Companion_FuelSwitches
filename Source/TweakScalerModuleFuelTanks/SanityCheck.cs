...
namespace TweakScaleCompanion.FuelSwitches.ModuleFuelTanks
{
	...
	internal class SanityCheck : MonoBehaviour
	{
		...
		private IEnumerator SanityCheckCoroutine()
		{
			...
			foreach (AvailablePart p in PartLoader.LoadedPartsList)
			{
				...
				{ 
					...
					{ 
						...
						try 
						{
							containsTarget = prefab.Modules.Contains(Startup.TARGET_MODULE_NAME) || prefab.Modules.Contains(Startup.SCALER_MODULE_NAME);
							++total_count;
					...
					try
					{
						...
						if (containsTarget)
						{
							if (null != (due = this.checkForInventory(prefab)))
							{
								...
								if (prefab.Modules.Contains(Startup.TARGET_MODULE_NAME))
									prefab.RemoveModule(prefab.Modules[Startup.TARGET_MODULE_NAME]);
								if (prefab.Modules.Contains(Startup.SCALER_MODULE_NAME))
									prefab.RemoveModule(prefab.Modules[Startup.SCALER_MODULE_NAME]);
							}
			...
			Log.info("SanityCheck Concluded : {0} parts found ; {1} parts using {2} ; {3} show stoppers detected .", total_count, parts_with_buoyancy_count, Startup.TARGET_MODULE_NAME, showstoppers_count);
		...
		{
			Log.dbg("Checking {0} Sanity for {1} at {2}", Startup.SCALER_MODULE_NAME, p.name, (null != p.partInfo ? p.partInfo.partUrl : "<no partInfo>"));

			if (!p.Modules.Contains(Startup.TARGET_MODULE_NAME)) return MSG_PART_DOES_NOT_NEED;
			if (!p.Modules.Contains(Startup.SCALER_MODULE_NAME)) return MSG_PART_IMPROPERLY_PATCHED;
